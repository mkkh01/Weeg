...
LONG signals are blocked in `RANGING` by default. The entry-quality layer rejects a LONG near the recent 20-candle high, near the wider 50-candle resistance, after a signal candle larger than the configured ATR limit, or when price is too extended above EMA20. By default, a valid unfilled bullish FVG is treated as the priority entry path only after a recent touch/retest; it is not mandatory. When no FVG is available, LONG may use the fallback confirmation path only when the market is trending up and momentum and volume are confirmed. Stops now prefer the latest structural swing low/high with an ATR buffer and targets are recalculated from the actual stop distance to preserve R:R. These thresholds are configurable through `LONG_PREFER_FVG`, `LONG_PEAK_DISTANCE_ATR`, `LONG_MAX_EXTENSION_ATR`, `LONG_RESISTANCE_DISTANCE_ATR`, and `LONG_MAX_SIGNAL_RANGE_ATR`, and are applied consistently to live analysis and backtests.
