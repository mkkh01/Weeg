...
LONG signals are blocked in `RANGING` by default. The entry-quality layer also rejects a LONG when the current price is within `0.35 × ATR` of the recent 20-candle high or when price is more than `1.5 × ATR` above EMA20. By default, a valid unfilled bullish FVG is treated as the priority entry path, but it is not mandatory. When no FVG is available, LONG may use the fallback confirmation path only when the market is trending up and momentum and volume are confirmed. These thresholds are configurable through `LONG_PREFER_FVG`, `LONG_PEAK_DISTANCE_ATR`, and `LONG_MAX_EXTENSION_ATR`, and are applied consistently to live analysis and backtests.
